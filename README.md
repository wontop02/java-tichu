...
- 라지 티츄와 스몰 티츄는 8장만 보고 부르는지, 14장을 모두 보고 부르는지의 차이다. 라지 티츄가 위험성이 높은 만큼, 성공했을 때 보너스 점수가 크다.
...
- 일반 족보는 6가지가 있다.
    ...
    2. **페어**: 동일한 등급의 카드 2장
    3. **트리플**: 동일한 등급의 카드 3장
    4. **풀하우스**: 페어와 트리플의 조합. 트리플의 등급을 기준으로 강함을 판단
    5. **연속 페어**: 연속된 페어의 조합. 가장 높은 등급의 페어를 기준으로 강함을 판단
        - 예) 2,2,3,3,4,4
    6. **스트레이트**: 5장 이상의 연속된 등급의 조합. 가장 높은 등급을 기준으로 강함을 판단
        - 예) 2,3,4,5,6,7
...
- 폭탄 족보는 2가지가 있다.
    1. **포 카드**: 동일한 등급의 카드 4장
    2. **스트레이트 플러쉬**: 동일한 문양의 5장 이상 스트레이트
