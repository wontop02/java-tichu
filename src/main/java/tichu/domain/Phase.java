...
public class Phase {
    ...
    public void evaluateCombination(Player player, Combination combination, Round round) {
        ...
        if (lastCombination == null) {
            firstCombination(player, combination);
            return;
        }
        validateNewCombination(combination);
    ...
    private void firstCombination(Player player, Combination combination) {
    ...
    private void validateNewCombination(Combination combination) {
        if (lastCombination.isBomb() && !combination.isBomb()) {
            throw new IllegalArgumentException(LOW_OR_SAME_COMBINATION);
        }
        // 싱글 비교
        if (lastCombination.getCombinationType() == CombinationType.SINGLE
                && combination.getCombinationType() == CombinationType.SINGLE) {
    ...
    private int evaluatePhoenixSingleCombination(int phoenixIndex, Card newTopCard) {
        ...
        return Integer.compare(beforePhoenixRank, combinationRank);
    }
    ...
    public void useBomb(String name, List<Card> cards, Round round) {
        ...
        registerCombination(player, bombCombination);
    }
